...
async function fetchChart(
  ...
  const { projectId, regionIndex } = creds();
...
export type TopvisorImportResult = {
  domain: string;
  rows: number;
};
...
export async function importTopvisorVisibility(
  days: number
): Promise<{ imported: number; results: TopvisorImportResult[] }> {
  ...
  const charts = [
    await fetchChart(null, date1, date2),
    await fetchChart(enabled.map((c) => c.id), date1, date2),
  ];

  let imported = 0;
  const results: TopvisorImportResult[] = [];
    ...
    for (const [seriesId, series] of Object.entries(chart.seriesByProjectsId)) {
      ...
      for (let i = 0; i < dates.length; i++) {
        ...
        await prisma.visibilityData.upsert({
          where: { projectId_date: { projectId: ourId, date } },
          create: { projectId: ourId, date, ...data },
          update: data,
        });
        rows++;
      }
      imported += rows;
      results.push({ domain, rows });
    }
  }
